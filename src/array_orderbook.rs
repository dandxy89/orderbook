...
impl<const N: usize> MetricsCalculator for ArrayOrderbook<N> {
...
impl Default for ArrayOrderbook<300> {
    #[inline]
    #[must_use]
    fn default() -> Self {
        Self::new()
...
mod test {
    ...
    fn bbo() {
        let mut lob = ArrayOrderbook::<3>::new();
    ...
    fn test_orderbook_metrics() {
        let mut lob = ArrayOrderbook::<5>::new();
    ...
    #[test]
    fn test_roatating_price() {
        let mut ob: ArrayOrderbook<10> = ArrayOrderbook::new();
...
mod sequence_tests {
    ...
    use crate::{array_orderbook::ArrayOrderbook, event::Event, event_kind::EventKind, interface::OrderBook as _, side::Side};

    #[test]
    /// Test that events with sequence IDs are processed in order and out-of-order
    /// events are ignored
    fn test_sequence_order() {
        let mut ob = ArrayOrderbook::<5>::new();

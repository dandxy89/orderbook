...
impl DecimalType for FixedDecimal {
    ...
    const ONE: Self = Self { raw: Self::SCALE_FACTOR };
    const TWO: Self = Self { raw: 2 * Self::SCALE_FACTOR };
    const MAX: Self = Self { raw: 1_000_000 * Self::SCALE_FACTOR };
    const MIN: Self = Self { raw: -1_000_000 * Self::SCALE_FACTOR };
    const ONE_HUNDRED: Self = Self { raw: 100 * Self::SCALE_FACTOR };
}
...
impl Sum for FixedDecimal {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(FixedDecimal::ZERO, Add::add)
    }
}

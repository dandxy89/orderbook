...
fn setup<T: OrderBook<Decimal> + Default>() -> T {
...
fn bench_array_l2_updates(bencher: Bencher) {
    bencher.with_inputs(setup::<ArrayOrderbook<300, Decimal>>).bench_refs(|ob| {
        for i in 0..10_000 {
...
fn bench_btree_l2_updates(bencher: Bencher) {
    bencher.with_inputs(setup::<BTreeOrderBook<Decimal>>).bench_refs(|ob| {
        for i in 0..10_000 {
            let (price, size) = generate_price_size(i);
            black_box(ob.process(Event::new(
                EventKind::L2,
